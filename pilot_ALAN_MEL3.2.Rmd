...
lnRRVCV2 <- metafor::vcalc(vi = lnRRV, cluster = StudyID, obs = obs_ID, time1 = Melatonin_sample_time2, data = dat, phi = 0.5, rho = 0.5)

# new zt
dat <- dat %>% mutate(ZT = case_when(Melatonin_sample_time2<=4 ~ "1",
                                     Melatonin_sample_time2>4 & Melatonin_sample_time2<=8 ~ "2",
                                     Melatonin_sample_time2>8 & Melatonin_sample_time2<=12 ~ "3",
                                     Melatonin_sample_time2>12 & Melatonin_sample_time2<=16 ~ "4",
                                     Melatonin_sample_time2>16 & Melatonin_sample_time2<=20 ~ "5",
                                     Melatonin_sample_time2>20 & Melatonin_sample_time2<=24 ~ "6"))
dat$ZT <- as.numeric(dat$ZT)
lnRRVCV3 <- metafor::vcalc(vi = lnRRV, cluster = StudyID, obs = obs_ID, time1 = ZT, data = dat, phi = 0.5, rho = 0.5)
# plot data
with(dat, interaction.plot(ZT, StudyID, lnRR, type="b", pch=19, lty="solid", xaxt="n", legend=FALSE, xlab="ZT", ylab="lnRR", bty="l"))


# alternative way
lnRRVCV4 <- impute_covariance_matrix(vi = dat$lnRRV, cluster = dat$StudyID, ar1 = 0.5, ti = dat$Melatonin_sample_time2, r = 0.5)


cov2cor(lnRRVCV2)[dat$StudyID %in% c("Aulsebrook_2020"), dat$StudyID %in% c("Aulsebrook_2020")]

cov2cor(lnRRVCV2)[dat$StudyID %in% c("Bishnupuri_2000"), dat$StudyID %in% c("Bishnupuri_2000")]
...
p3 <- facet_plot(p2, panel = 'N observations', data = fe.spp.lnRR2, 
				geom = geom_barh, 
  ...
  geom_facet(panel = "Estimates of mean ratio (lnRR)", data = fe.spp.lnRR2,
  ...
  geom_facet(panel = "Estimates of CV ratio (lnCVR)", data = fe.spp.lnCVR2,
  ...
  geom_facet(panel = "Estimates of CV ratio (lnCVR)", data = fe.spp.lnRR2,
           geom = geom_vline, 
...
png(filename = "Figure 3 (Phy_est).png", width = 8, height = 6, units = "in", type = "windows", res = 400)
p4
dev.off()
...
names(df2) <- c("Melaotnin location", "Activity time", "Living conditions", "Light colour", "Light phase", "Light intensity", "Light wavelength", "ALAN duration", "Randomization", "Obs. blinding","Sex","Sampling time","Measurement method","Class")

df2 %>% gg_miss_var() + theme_bw() +
  labs(x = " ", y  = "Number of effect sizes with missing information") -> missness_plot
...
df2 %>% gg_miss_fct(fct = Class) + labs(y = "") + guides(fill = guide_legend(title = "%")) -> missness_plot2
...
# replace NA
df1$grouped_tissue <- ifelse(is.na(df1$grouped_tissue), "Not reported", df1$grouped_tissue)
df1$Light_colour <- ifelse(is.na(df1$Light_colour), "Not reported", df1$Light_colour)
df1$Night_Phase_category <- ifelse(is.na(df1$Night_Phase_category), "Not reported", df1$Night_Phase_category)
...
df1$grouped_tissue <- factor(df1$grouped_tissue, levels = c("Central Mel", "Peripheral Mel","Not reported"))
df1$Light_colour <- factor(df1$Light_colour, levels = c("Blue","Yellow","Red","White","Not reported"))
df1$Night_Phase_category <- factor(df1$Night_Phase_category, levels = c("Early night","Mid night","Late night","All night","Not reported"))
# replace Actinopterygii by the «superclass» Osteichthyes - Ref2's comment
df1$Class[df1$Class == "Actinopterygii"] <- "Osteichthyes"

df <- df1 %>%
  ggsankey::make_long(Class,grouped_tissue,Species_diurnal_nocturnal,Captive_or_Free_living,Light_colour,Night_Phase_category)
...
scale_x_discrete(labels = c("Class","Melatonin location", "Activity times", "Living conditions", "Light colour", "Light phase"), position = "top")
dev.off()
...
colnames(data2)[colnames(data2)=="NA_"] <- "No reported"
...
# reorder dataframe - https://www.statology.org/sort-dataframe-by-column-in-r/
#data2[order(-data2$Freq), ]

#reorder(predictor, weight)

#names(data2) <- c("Class","Light colour","Freq")
# convert into factor
#data2[c("Class","Light colour")] <- lapply(data2[c("Class","Light colour")], factor)
# check conversion
#sapply(data2, class)
# relevel to make the best-representative color to appear as the first
#data2$`Light colour` <- factor(data2$`Light colour`, levels = c("Yellow", "Red",  "Blue", "White"))
#data2$Class <- factor(data2$Class, levels = c("Mammalia", "Aves", "Osteichthyes", "Reptilia", "Actinopterygii", "Amphibia"))
...
a <- rma.mv(yi = lnRR, V = lnRRVCV, random = list(~ 1 | Spp, ~1 | Phy, ~1 | StudyID, ~1 | obs_ID), method = "REML", test = "t", data = dat, sparse = T, R = list(Phy=tr_matrix), verbose = T)
a2 <- rma.mv(yi = lnRR, V = lnRRVCV2, random = list(~ 1 | Spp, ~1 | Phy, ~1 | StudyID, ~1 | obs_ID), method = "REML", test = "t", data = dat, sparse = T, R = list(Phy=tr_matrix), verbose = T)
a3 <- rma.mv(yi = lnRR, V = lnRRVCV3, random = list(~ 1 | Spp, ~1 | Phy, ~1 | StudyID, ~1 | obs_ID), method = "REML", test = "t", data = dat, sparse = T, R = list(Phy=tr_matrix), verbose = T)


b <- rma.mv(yi = lnRR, V = lnRRVCV3, random = list(~ 1 | Spp, ~ZT | StudyID, ~1 | obs_ID), mods = ~ factor(ZT) - 1, struct = "AR", method = "REML", test = "t", data = dat, sparse = T, verbose = T)
robust(b, cluster = dat$StudyID)


b2 <- rma.mv(yi = lnRR, V = lnRRVCV3, random = list(~ 1 | Spp, ~ZT | StudyID, ~1 | obs_ID), mods = ~ factor(ZT) - 1, struct = "HAR", method = "REML", test = "t", data = dat, sparse = T, verbose = T)
robust(b2, cluster = dat$StudyID)

b3 <- rma.mv(yi = lnRR, V = lnRRV, random = list(~ 1 | Spp, ~ZT | StudyID, ~1 | obs_ID), mods = ~ factor(ZT) - 1, struct = "AR", method = "REML", test = "t", data = dat, sparse = T, verbose = T)
robust(b3, cluster = dat$StudyID)

b4 <- rma.mv(yi = lnRR, V = lnRRV, random = list(~ 1 | Spp, ~ZT | StudyID, ~1 | obs_ID), mods = ~ factor(ZT) - 1, struct = "HAR", method = "REML", test = "t", data = dat, sparse = T, verbose = T)
robust(b4, cluster = dat$StudyID)

#b3 <- rma.mv(yi = lnRR, V = lnRRVCV3, random = list(~ 1 | Spp, ~ZT | StudyID, ~1 | obs_ID), mods = ~ factor(ZT) - 1, struct = "CAR", method = "REML", test = "t", data = dat, sparse = T, verbose = T)
...
robust(MLMR_6N.lux_lnRR, cluster = dat_N_lux$StudyID)
...
png(filename = "Figure 7 (model_selection).png", width = 6.5, height = 4, units = "in", type = "windows", res = 400)
vip_predictors %>% dfround(3)  %>%
  ggplot(aes(x = reorder(predictor, weight), y = weight), fill = "lightgrey") +
  ...
  scale_x_discrete(labels = c("Exposure duration of ALAN", "Activity time of Spp","Living condition of Spp","Location of Mel","Light intensity","Light phase","Light color"))
dev.off()
...
These are additional analyses that were required by peer-review referees

## Normality check

```{r}
png(filename = "Figure (Normality check lnRR).png", width = 8, height = 8, units = "in", type = "windows", res = 400)
# define plotting region
par(mfrow=c(2,2)) 
# create histogram
hist(exp(dat$lnRR), col='steelblue', main='Before log-transformation', xlab="Response ratio (RR)")
hist(dat$lnRR, col='steelblue', main='After log-transformation',xlab="Log response ratio (lnRR)")
# create Q-Q plot 
qqnorm(exp(dat$lnRR), main='Before log-transformation')
#qqline(exp(dat$lnRR))
qqnorm(dat$lnRR, main='After log-transformation')
#qqline(dat$lnRR)
dev.off()
...
```{r}
# subset of nighttime melatonin using lux<100
dat_N_lux100 <- filter(dat_N_lux,Night_light_intensity_exp<100)

# VCV
lnRRVCV_N_lux100 <- metafor::vcalc(vi = lnRRV, cluster = StudyID, obs = obs_ID, data = dat_N_lux100, rho = 0.5)

# tree
taxa_N_lux100 <- tnrs_match_names(names = unique(dat_N_lux100$Species), context = "Animals")
my.tr_N_lux100 <- tol_induced_subtree(ott_ids=ott_id(taxa_N_lux100))
my.tr_N_lux100$tip.label <- strip_ott_ids(my.tr_N_lux100$tip.label, remove_underscores = TRUE)
is.binary(my.tr_N_lux100) 
my.tr_N_lux100 <- compute.brlen(my.tr_N_lux100, method = "Grafen", power = 1)
is.ultrametric(my.tr_N_lux100)
tr_matrix_N_lux100 <- vcv.phylo(my.tr_N_lux100, model = "Brownian", corr = T)

# visualize correlation
ggcorrplot::ggcorrplot(tr_matrix_N_lux100, sig.level=0.05, lab_size = 4.5, p.mat = NULL, insig = c("pch", "blank"), pch = 1, pch.col = "black", pch.cex =1, tl.cex = 14) +
  theme(axis.text.x = element_text(size = 10, margin=margin(-2,0,0,0)),
        axis.text.y = element_text(size = 10, margin=margin(0,-2,0,0)),
        panel.grid.minor = element_line(size=10)) + 
  geom_tile(fill="white") +
...
# plot tree
plot.phylo(vcv2phylo(tr_matrix_N_lux100), cex = 0.6, label.offset = 0.1, no.margin = T)



# sensitivity analysis required by one peer-review referee
MLMR_6N.lux100_lnRR <- rma.mv(yi = lnRR, V = lnRRVCV_N_lux100, random = list(~ 1 | Spp, ~1 | Phy, ~1 | StudyID, ~1 | obs_ID), mods = ~ log(Night_light_intensity_exp), method = "REML", test = "t", data = dat_N_lux100, sparse = T, R = list(Phy=tr_matrix_N_lux100), verbose = T)
summary(MLMR_6N.lux100_lnRR) #robust(MLMR_6N.lux100_lnRR, cluster = dat_N_lux100$StudyID)
...
```{r}
# white light subset
dat_WL <- filter(dat, Light_colour == "White" & Spectral_characteristics == "Full-spectrum")
# CCT
dat_WL$Light_CT
```
